...
* **미디어 뷰어 & 연관 포스트 (Media Viewer):** 부모/자식(Parent/Child) 관계의 연관 포스트를 묶어서 볼 수 있는 버튼을 제공합니다. 썸네일에 영상 길이가 직관적으로 표시되며 스와이프 페이징, 핀치 줌 전체화면, WebM/MP4 비디오 플레이어(ExoPlayer)를 내장하고 있습니다.

* **편리한 환경 설정 & 자동 업데이트 (Management & Updates):** 앱 설정에서 실시간으로 캐시 용량을 확인하고 비울 수 있습니다.
...
* **Media Viewer & Related Posts:** Provides an integrated search button for Parent/Child related posts.
Displays video durations directly on thumbnails, and features swipe paging, pinch-to-zoom fullscreen viewing, and a built-in WebM/MP4 video player powered by ExoPlayer.

* **Cache Management & Auto-Updates:** Check and clear the app's cache size in real-time from the settings.
